...
public class GeneralUtilVersionData {
	...
	private String updateSysConfExcelData() {
		...
		for (Map<String, Object> map : listMapData) {
			...
			String excelName = (String) map.get("SYSCONFEXCELDATANAME");
			...
			script.append(
					"update fg_s_sysconfexceldata_pivot t set t.EXCELDATA = (select max(t1.id) from FG_SEQUENCE_FILES t1 where t1.formcode = 'SysConfExcelData.excelFile') where SYSCONFEXCELDATANAME = '"
							+ excelName + "';\n");
