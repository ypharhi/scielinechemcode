(function () {
    ...
    var DropDownDialog = (function (_super) {
            ...
            switch (type) {
                ...
                case GC.Spread.Sheets.DropDownType.slider:
                    ...
                    if (max.val() !== undefined && max.val() !== "") {
                        sliderOption.max = parseFloat(max.val());
                    }
                    if (min.val() !== undefined && min.val() !== "") {
                        sliderOption.min = parseFloat(min.val());
                    }
                    if (step.val() !== undefined && step.val() !== "") {
                        sliderOption.step = parseFloat(step.val());
                    }
        ...
        DropDownDialog.prototype._getListResult = function () {
            ...
            result.multiSelect = this._element.find(".list-config").find("#dropdownListMultiSelect").prop("checked");
            var valueType = this._element.find(".list-config").find(".dropdownListValue");
            if (valueType.val() !== undefined && valueType.val() !== "") {
                result.valueType = parseInt(valueType.val());
            }
        ...
        DropDownDialog.prototype._render = function (config) {
            ...
            switch (this.type) {
                ...
                case GC.Spread.Sheets.DropDownType.timePicker:
                    ...
                    container.find(".can-input-select select").val(timeOption.formatString); //Fix SJS-2246 Can't select the first item of the format combo.
                ...
                case GC.Spread.Sheets.DropDownType.slider:
                    ...
                    var sliderFormatString = container.find(".formatString");
                    var marks = container.find(".marks");
                    max.spinner("option", "step", sliderOption.max / 2 || 10); // The value can't set when it's smaller than step, so here use a trick.
                    max.spinner('value', isNullOrUndefined(sliderOption.max) ? 100 : sliderOption.max);
                    min.spinner("option", "step", sliderOption.min / 2 || 1);
                    min.spinner('value', isNullOrUndefined(sliderOption.min) ? 0 : sliderOption.min);
                    step.spinner("option", "step", sliderOption.step / 2 || 1);
                    step.spinner('value', isNullOrUndefined(sliderOption.step) ? 1 : sliderOption.step);
                    ...
                    container.find(".can-input-select select").val(sliderOption.formatString); //Fix SJS-2246 Can't select the first item of the format combo.
        ...
        DropDownDialog.prototype._renderColorOption = function (option) {
            ...
            var standardColor = option.groups[1].colors[0];
            var colorItem;
            for (var i = 0; i < 10; i++) {
                var themeContainer = container.find("#theme-color-" + i);
                ...
                var stardardColorSpan = stardardContainer.find(".color-span");
                colorItem = designer.ColorHelper.parse(standardColor[i], designer.wrapper.spread.getActiveSheet().currentTheme().colors());
        ...
        DropDownDialog.prototype._renderListOption = function (option) {
            ...
            container.find(".dropdownListValue").val(option.valueType ? 1 : 0);
            container.find("#dropdownListMultiSelect").prop("checked", option.multiSelect ? option.multiSelect : false);
    ...
    var FormatDialog = (function (_super) {
        ...
        FormatDialog.prototype._beforeOpen = function (args) {
            ...
            this._updateCheckBoxValue('.input-ellipsis', this._cacheStyle['showEllipsis']());
        ...
        FormatDialog.prototype._init = function () {
            ...
            this._element.find('.input-ellipsis').click(function () {
                var value = self._element.find('.input-ellipsis').prop('checked');
                self._cacheStyle['showEllipsis'](value);
            });
        ...
        FormatDialog.prototype._returnFormat = function () {
            ...
            return {
                ...
                cellButtons: style.cellButtons,
                dropDowns: style.dropDowns,
        ...
        FormatDialog.prototype._createDateSetting = function () {
            ...
            var eraCheckbox = this._getEraCheckbox("showEraFirstYear", "showEraFirstYear", "showEraFirstYearDiv");
            this._element.find('.tab-number-setting').append(typeSelect).append(localeCulture).append(calendar).append(eraCheckbox);
            this._element.find(".tab-number-comments").append(para);
            ...
            switch (locale) {
                case "ja_jp":
                    ...
                    if (calendar === "western") {
                        ...
                        this._element.find('.showEraFirstYearDiv').removeClass("hidden");
                        var JERFormats = designer.res.japanEmperorReignDateFormat;
                        for (i = 0; i < JERFormats.length; i++) {
                            if (this.isShowEraFirstYear && i === 1) {
                                dates.push(this._getTextBySpread(defaultDate, JERFormats[1].replace("[$-411]", "[$-ja-JP-x-gannen]")));
                            } else {
                ...
                case "en_us":
                    ...
                    this._element.find('.showEraFirstYearDiv').addClass("hidden");
        ...
        FormatDialog.prototype._getEraCheckbox = function (className, idName, categoryClass) {
            var self = this;
            var showEraFirstYearDiv = $('<div></div>').addClass("gcui-numberpicker hidden " + categoryClass);
            $('<input>').attr({
                'id': idName,
                'type': 'checkbox'
            }).addClass(className).appendTo(showEraFirstYearDiv).change(function () {
                self.isShowEraFirstYear = $(this).is(':checked');
                self._refreshDateSample();
            });
            $('<label></label>').attr('for', idName).text(designer.res.formatNumberPickerSetting.showEraFirstYear).appendTo(showEraFirstYearDiv);
            return showEraFirstYearDiv;
        };
            ...
            switch (localeName) {
                case "ja_jp":
                    ...
                    if (calendarName === "western") {
                        ...
                        this._element.find('.showEraFirstYearDiv').removeClass("hidden");
                        var allFormats = JSON.parse(JSON.stringify(designer.res.japanEmperorReignDateFormat));
                        if (this.isShowEraFirstYear) {
                            allFormats[1] = allFormats[1].replace("[$-411]", "[$-ja-JP-x-gannen]")
                        }
                    }
                    break;
                case "en_us":
        ...
        FormatDialog.prototype._getTextBySpread = function (value, format) {
            ...
            if (format.charAt(0) === "=" || format.indexOf("{{") >= 0 || format.indexOf("}}") >= 0) {
                return value;
            }
        ...
        FormatDialog.prototype._parseFormat = function (format) {
            ...
            var dateJapanJERFirstYearFormats = designer.res.japanEmperorReignFirstYearDateFormat;
            ...
            if (dateFormat) {
                ...
                if (dateIndex >= 0) {
                    this.isShowEraFirstYear = true;
                    this._element.find('#showEraFirstYear').prop("checked", true);
                    ...
                    this._element.find('.date-type-select').trigger("change");
                    return;
                } else if ((dateIndex = $.inArray(dateFormat, dateJapanJERFormats)) >= 0 || (dateIndex = $.inArray(dateFormat, dateJapanJERFirstYearFormats)) >= 0) {
                    this.isShowEraFirstYear = $.inArray(dateFormat, dateJapanJERFirstYearFormats) === 1;
                    this._element.find('#showEraFirstYear').prop("checked", this.isShowEraFirstYear);
                    this._element.find('.date-type-locale-select').val("ja_jp").trigger('change');
