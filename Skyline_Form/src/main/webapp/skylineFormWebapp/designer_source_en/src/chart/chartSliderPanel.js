(function () {
    ...
    var chartDescription = {
        'baseChart': {
            ...
            dataLabels: {
                fillAndLine: { Line: ['fill', 'border'] },
                seriesOptions: { LabelOptions: ['labelOptions', 'labelFormat'] },
            ...
            legend: {
                fillAndLine: { Line: ['fill', 'border'] },
                legendOptions: { LegendOptions: ['position', 'legendLayout'] },
                textFill: { Text: ['textFill', 'font'] }
            },
            primaryCategory: {
    ...
    var datalabel = designer.res.chartSliderPanel.dataLabels;
    var comma = datalabel.comma;
    var semicolon = datalabel.semicolon;
    var period = datalabel.period;
    var newLine = datalabel.newLine;
    var space = datalabel.space;
    const separatorListResvered = {
        ", ": comma,
        "; ": semicolon,
        ". ": period,
        "\n": newLine,
    ...
    const datalabelPosition = [datalabel.bestFit, datalabel.below, datalabel.center, datalabel.insideBase, datalabel.insideEnd, datalabel.left, datalabel.outsideEnd, datalabel.right, datalabel.above];
    ...
    function _buildPanelContentCss(title) {
        var cssContent = title && title.split('And');
        var css = '';
        if (cssContent.length > 0) {
    ...
    function getTrendlineData(chart, outDataObj, category) {
        ...
        function getFontOptions(trendline) {
            var chartAreaStyle = chart.chartArea();
            return {
                name: designer.res.chartSliderPanel.font,
                data: {
                    fontSize: trendline.fontSize ? trendline.fontSize : (chartAreaStyle.fontSize || 11),
                    fontFamily: trendline.fontFamily ? trendline.fontFamily : (chartAreaStyle.fontFamily || 'Calibri')
                },
                type: 'font',
                resources: {
        ...
        function getTextFill(trendline, seriesIndex, count) {
            var chartAreaStyle = chart.chartArea();
            return {
                name: designer.res.chartSliderPanel.textFill,
                data: {
                    color: trendline.fontColor ? trendline.fontColor : chartAreaStyle.color,
                    colorStr: 'fontColor'
                },
                type: 'textFill',
                resources: {
    ...
    function getDataLabels(chart, outDataObj, category) {
        var seriesCollection = chart.series(), seriesCount = seriesCollection.Count;
        var seriesOfchart;
        var getProperty = {
            'fill': getFill,
        ...
        for (var index = 0; index < seriesCount; index++) {
            var dataLabels = seriesCollection.get(index).dataLabels;
            var isShow = dataLabels.showSeriesName || dataLabels.showCategoryName || dataLabels.showPercentage || dataLabels.showValue;
            if (dataLabels && isShow && outDataObj["series"][index]) {
                var dataObj = getEditorData(dataLabels, chart, index);
                dataObj["name"] = category[index];
                dataObj["index"] = index;
                outDataObj["series"][index].dataLabels = dataObj
            }
        }
        function getEditorData(dataLabels, chart, seriesIndex) {
            var dataObj = {};
            seriesOfchart = chartDescription['baseChart']['dataLabels'];
            for (var i in seriesOfchart) {
                var data = dataObj[i] = { type: i };
                var content = {};
                for (var j in seriesOfchart[i]) { /* NOSONAR: Forin*/
                    var editors = content[j] = [];
                    seriesOfchart[i][j].forEach(function (k) {
                        var editor = getProperty[k](dataLabels, chart, seriesIndex);
                        editors.push(editor);
                    });
                }
                data['content'] = [content];
        ...
        function getFill(dataLabels) {
            return {
                name: designer.res.chartSliderPanel.fill,
                data: {
                    backColor: dataLabels.backColor,
                    transparency: dataLabels.backColorTransparency
                },
                type: 'fill',
                resources: {
                    noColorText: designer.res.chartSliderPanel.noFill,
        ...
        function getBorder(dataLabels) {
            return {
                name: designer.res.chartSliderPanel.border,
                data: {
                    width: dataLabels.borderWidth,
                    color: dataLabels.borderColor,
                    transparency: dataLabels.borderColorTransparency,
                    colorStr: "borderColor",
                    widthStr: "borderWidth",
                    transparentStr: "borderColorTransparency",
                },
                type: 'border',
                resources: {
                    noColorText: designer.res.chartSliderPanel.noLine,
        ...
        function getTextFill(dataLabels) {
            return {
                name: designer.res.chartSliderPanel.textFill,
                data: {
                    color: dataLabels.color,
                    transparency: dataLabels.transparency,
                }, //legend text transparency is not API, keep it undefined.
                type: 'textFill',
                resources: {
                    noColorText: designer.res.chartSliderPanel.noFill,
                    solidColorText: designer.res.chartSliderPanel.solidFill
        ...
        function getLabelOptions(dataLabels, chart, seriesIndex) {
            var positionItems;
            var chartType = chart.chartType();
            if (chartType === 0 /* combo */) {
                var series = chart.series().get(seriesIndex);
                chartType = series.chartType;
            }
            var chartGroupStr = chartHelper.getChartGroupString(chartType);
            var chartTypeStr = chartHelper.getChartTypeString(chartType);
            var isStackedChart = chartHelper.isStackedChart(chartType);
            var dataLabelsRes = designer.res.chartSliderPanel.dataLabels;
            var center = dataLabelsRes.center;
            var insideEnd = dataLabelsRes.insideEnd;
            var outsideEnd = dataLabelsRes.outsideEnd;
            var bestFit = dataLabelsRes.bestFit;
            var below = dataLabelsRes.below;
            var above = dataLabelsRes.above;
            switch (chartGroupStr) {
                case "BarGroup":
                case "ColumnGroup":
                    positionItems = [center, insideEnd, outsideEnd];
                    break;
                case "StockGroup":
                case "ScatterGroup":
                case "LineGroup":
                case "ComboGroup":
                    positionItems = [center, above, below];
                    break;
                case "PieGroup":
                    positionItems = [center, insideEnd, outsideEnd, bestFit];
                    break;
            }
            if (chartTypeStr === 'doughnut') {
                positionItems = keyword_undefined;
            }
            if (isStackedChart && positionItems && positionItems.indexOf(outsideEnd) > -1) {
                positionItems.splice(positionItems.indexOf(outsideEnd), 1);
            }
            return {
                name: designer.res.chartSliderPanel.dataLabels.labelOptions,
                data: {
                    ...
                    "separator": dataLabels.separator,
                },
                type: 'dataLabelsOptions',
                resources: {
                    showCategoryName: designer.res.chartSliderPanel.dataLabels.showCategoryName,
                    showSeriesName: designer.res.chartSliderPanel.dataLabels.showSeriesName,
                    showValue: designer.res.chartSliderPanel.dataLabels.showValue,
                    showPercentage: designer.res.chartSliderPanel.dataLabels.showPercentage,
                    separator: designer.res.chartSliderPanel.dataLabels.separator,
                    position: designer.res.chartSliderPanel.dataLabels.labelPosition,
                    positionItems: positionItems,
                    labelPositionText: designer.res.chartSliderPanel.dataLabels.labelPositionText,
                    labelContainsText: designer.res.chartSliderPanel.dataLabels.labelContainsText,
                }
            };
        }
        function getLabelFormat(dataLabels) {
            return {
                name: designer.res.chartSliderPanel.dataLabels.numberFormat,
                data: {
                    formatCode: dataLabels.format
                },
                type: 'dataLabelsNumberFormat',
                resources: {
                    formatText: designer.res.chartSliderPanel.dataLabels.format,
                }
            };
        }
    }
        ...
        function getBorder(series1) {
            return {
                ...
                data: {
                    ...
                    lineType: series1.border.lineType,
                    colorStr: "borderColor",
                    widthStr: "borderWidth",
                    transparentStr: "borderTransparency",
                    lineTypeStr: "borderLineType"
                },
                type: 'border',
                resources: {
                    noColorText: designer.res.chartSliderPanel.noLine,
    ...
    function getLegendData(chart) {
        ...
        function getPosition(legend1) {
            return {
                ...
                data: { position: legend1.position, overlapping: legend1.showLegendWithoutOverlapping },
                type: 'legend',
                resources: {
                    ...
                    overlapping: designer.res.chartSliderPanel.legend.overlapping
                }
            };
        }
        ...
        function getLayout(legend1) {
            return {
                name: designer.res.chartSliderPanel.legend.layout,
                data: { layout: legend1.layout || { x: undefined, y: undefined, width: undefined, height: undefined } },
                type: 'legendLayout',
                resources: {
                    x: designer.res.chartSliderPanel.legend.x,
                    y: designer.res.chartSliderPanel.legend.y,
                    width: designer.res.chartSliderPanel.legend.width,
                    height: designer.res.chartSliderPanel.legend.height
                }
            }
        }
    }
    ...
    function getAxisData(chart, outDataObj) {
        var axes = chart.getAxesImp(true);
        ...
        function getAxisCategoryOptions(axes1) {
            ...
            return {
                ...
                data: {
                    ...
                    majorUnit: axes1.majorUnit,
                    majorUnitScale: axes1.majorUnitScale,
                    minorUnit: axes1.minorUnit,
                    minorUnitScale: axes1.minorUnitScale,
                    baseUnit: axes1.baseUnit,
                },
                type: 'axisCategory',
                resources: {
                    ...
                    maxValueText: designer.res.chartSliderPanel.maximum,
                    minValueText: designer.res.chartSliderPanel.minimum,
                    majorUnitText: designer.res.chartSliderPanel.dateUnitsMajor,
                    minorUnitText: designer.res.chartSliderPanel.dateUnitsMinor,
                    baseUnitText: designer.res.chartSliderPanel.dateBaseUnit,
                }
            };
        }
    ...
    function AxisCategoryEditor(container, data, name, resources, categoryTypeChangedCallback) {
        ...
        this.settingValueEditor = new AxisValueEditor(container, {
            ...
            majorUnit: data.majorUnit,
            minorUnit: data.minorUnit,
            majorUnitScale: data.majorUnitScale,
            minorUnitScale: data.minorUnitScale,
            baseUnit: data.baseUnit,
            logBase: data.logBase
        }, name, {
            maxValueText: resources.maxValueText,
            minValueText: resources.minValueText,
            majorUnitText: resources.majorUnitText,
            minorUnitText: resources.minorUnitText,
            baseUnitText: resources.baseUnitText,
            displayUnitText: resources.displayUnitText,
    ...
    function AxisValueEditor(container, data, name, resources, valueChangedCallback, isAxisCategory) {
        this.valueChanged = valueChangedCallback;
        var self = this;
        var isShowAuto = true;
        var isShowSelect = true;
        var isDateAxis = data.categoryType === 2;
        if (!isAxisCategory) {
            isShowSelect = false;
        }
        if (isAxisCategory && isDateAxis) {
            self.boundsText = new ParagraphText(container, designer.res.chartSliderPanel.bounds);
        }

        self.maxValueEditor = new InputTextEditor(container, {
            resources: resources.maxValueText,
            text: data.maxValue
        }, function (num) {
            var majorNum = self.valueChanged('max', num);
            self.updateMaxUI(majorNum);
        }, isShowAuto);
        self.minValueEditor = new InputTextEditor(container, {
            resources: resources.minValueText,
            text: data.minValue
        }, function (num) {
        ...
        if (isDateAxis || !isAxisCategory) {
            self.unitsText = new ParagraphText(container, designer.res.chartSliderPanel.units);
            self.majorUnitEditor = new InputTextEditor(container, {
                resources: resources.majorUnitText,
                text: data.majorUnit,
                unitScale: data.majorUnitScale
            }, function (value, isUnitScale) {
                if (value !== null && value !== undefined && typeof value === 'string') {
                    value = Number(value);
                }
                if (!isUnitScale) {
                    var majorNum = self.valueChanged('majorUnit', value);
                    self.updateMajorUI(majorNum);
                } else {
                    var majorUnitScale = self.valueChanged('majorUnitScale', value);
                    self.updateMajorUnitScaleUI(majorUnitScale);
                }
            }, isShowAuto, isShowSelect);
            self.minorUnitEditor = new InputTextEditor(container, {
                resources: resources.minorUnitText,
                text: data.minorUnit,
                unitScale: data.minorUnitScale
            }, function (value, isUnitScale) {
                if (!isUnitScale) {
                    var majorNum = self.valueChanged('minorUnit', value);
                    self.updateMinorUI(majorNum);
                } else {
                    var minorUnitScale = self.valueChanged('minorUnitScale', value);
                    self.updateMinorUnitScaleUI(minorUnitScale);
                }
            }, isShowAuto, isShowSelect);
            if (isDateAxis) {
                self.baseUnitEditor = new InputTextEditor(container, {
                    resources: resources.baseUnitText,
                    unitScale: data.baseUnit
                }, function (value) {
                    if (value !== null && value !== undefined) {
                        value = Number(value);
                    }
                    var baseUnitScale = self.valueChanged('baseUnit', value);
                    self.updateBaseUnitScaleUI(baseUnitScale);
                }, isShowAuto, isShowSelect, true);
                self.updateMajorUnitScaleUI(data.majorUnitScale);
                self.updateMinorUnitScaleUI(data.minorUnitScale);
                self.updateBaseUnitScaleUI(data.baseUnit);
                data.majorUnit = data.majorUnit;
                data.minorUnit = data.minorUnit;
            }
            ...
            if (!isAxisCategory) {
                ...
                self.logAxisCheckBox = new Checkbox(container, {
                    text: resources.logarithmicScaleText,
                    appendSpan: this.logInput.inputSpan
                }, 'logAxis', function (checked) {
                    if (checked) {
    ...
    AxisValueEditor.prototype.updateMajorUI = function (value) {
        this.majorUnitEditor.actualText = value;
        this.majorUnitEditor.updateValue();
    };
    AxisValueEditor.prototype.updateMinorUI = function (value) {
        this.minorUnitEditor.actualText = value;
        this.minorUnitEditor.updateValue();
    };
    AxisValueEditor.prototype.updateMajorUnitScaleUI = function (value) {
        this.majorUnitEditor.actualUnitScale = value;
        this.majorUnitEditor.updateValue();
    };
    AxisValueEditor.prototype.updateMinorUnitScaleUI = function (value) {
        this.minorUnitEditor.actualUnitScale = value;
        this.minorUnitEditor.updateValue();
    };
    AxisValueEditor.prototype.updateBaseUnitScaleUI = function (value) {
        this.baseUnitEditor.actualUnitScale = value;
        this.baseUnitEditor.updateValue();
    };
    AxisValueEditor.prototype.updateMaxUI = function (value) {
        this.maxValueEditor.actualText = value;
        this.maxValueEditor.updateValue();
    };
    AxisValueEditor.prototype.updateMinUI = function (value) {
        this.minValueEditor.actualText = value;
        this.minValueEditor.updateValue();
    };
    AxisValueEditor.prototype.showEditor = function () {
        this.minValueEditor.show();
        this.maxValueEditor.show();
        if (this.majorUnitEditor || this.minorUnitEditor) {
            this.majorUnitEditor.show();
            this.minorUnitEditor.show();
            this.baseUnitEditor.show();
            this.unitsText.show();
            this.boundsText.show();
        }
    };
    AxisValueEditor.prototype.hideEditor = function () {
        this.minValueEditor.hide();
        this.maxValueEditor.hide();
        if (this.majorUnitEditor || this.minorUnitEditor) {
            this.majorUnitEditor.hide();
            this.minorUnitEditor.hide();
            this.baseUnitEditor.hide();
            this.unitsText.hide();
            this.boundsText.hide();
        }
    };
    ...
    function LegendPositionEditor(container, data, name, resources, legendPositionChangedCallbck) {
        ...
        this.overlapping = resources.overlapping;
        ...
        this.checkbox = new Checkbox(container, { text: this.overlapping }, name, function (element) {
            self.checkboxChanged(element);
        });
        this.updateUI('position', data.position);
        this.updateUI('overlapping', data.overlapping);
    }

    LegendPositionEditor.prototype.radioGroupSelectionChanged = function (value) {
        var result = this.changedPosition('position', value);
        if (result) {
            this.updateUI('position', result);
        }
    };
    LegendPositionEditor.prototype.checkboxChanged = function (value) {
        var result = this.changedPosition('overlapping', value);
        if (result) {
            this.updateUI('overlapping', result);
        }
    };
    LegendPositionEditor.prototype.updateUI = function (type, value) {
        switch (type) {
            case 'position':
        var selectedItems = [, this.topText, this.rightText, this.leftText, this.bottomText, this.topRightText];
                this.radiogroup.setSelectedItem(selectedItems[value]);
                break;
            case 'overlapping':
    ...
    function LegendLayoutEditor(container, data, name, resources, legendLayoutChangedCallbck) {
        this.legendLayoutChangedCallbck = legendLayoutChangedCallbck;
        var self = this;
        self.x = resources.x;
        self.y = resources.y;
        self.width = resources.width;
        self.height = resources.height;
        self.xEditor = new InputNumberEditor(container, self.x, /*{min: 0, max: 3, type: int}, data.borderWidth, */function (x) {
            self.updateXUI(x);
            self.changedLayout();
        }, { min: 0, max: 100, descString: '%', inputWidth: "55px" });
        self.yEditor = new InputNumberEditor(container, self.y, /*{min: 0, max: 3, type: int}, data.borderWidth, */function (y) {
            self.updateYUI(y);
            self.changedLayout();
        }, { min: 0, max: 100, descString: '%', inputWidth: "55px" });
        self.widthEditor = new InputNumberEditor(container, self.width, /*{min: 0, max: 3, type: int}, data.borderWidth, */function (width) {
            self.updateWidthUI(width);
            self.changedLayout();
        }, { min: 0, max: 100, descString: '%', inputWidth: "55px" });
        self.heightEditor = new InputNumberEditor(container, self.height, /*{min: 0, max: 3, type: int}, data.borderWidth, */function (height) {
            self.updateHeightUI(height);
            self.changedLayout();
        }, { min: 0, max: 100, descString: '%', inputWidth: "55px" });
        self.updateWidthUI(data.layout.width * 100);
        self.updateHeightUI(data.layout.height * 100);
        self.updateXUI(data.layout.x * 100);
        self.updateYUI(data.layout.y * 100);
    }
    LegendLayoutEditor.prototype.updateWidthUI = function (value) {
        this.widthEditor.setValue(value);
    };
    LegendLayoutEditor.prototype.updateHeightUI = function (value) {
        this.heightEditor.setValue(value);
    };
    LegendLayoutEditor.prototype.updateXUI = function (value) {
        this.xEditor.setValue(value);
    };
    LegendLayoutEditor.prototype.updateYUI = function (value) {
        this.yEditor.setValue(value);
    };
    LegendLayoutEditor.prototype.changedLayout = function () {
        var self = this;
        var layout = {
            x: self.xEditor.getValue() ? self.xEditor.getValue() / 100 : self.xEditor.getValue(),
            y: self.yEditor.getValue() ? self.yEditor.getValue() / 100 : self.yEditor.getValue(),
            width: self.widthEditor.getValue() ? self.widthEditor.getValue() / 100 : self.widthEditor.getValue(),
            height: self.heightEditor.getValue() ? self.heightEditor.getValue() / 100 : self.heightEditor.getValue()
        }
        self.legendLayoutChangedCallbck('layout', layout);
    };
    ...
    function TrendlineOptionEditor(container, data, name, resources, optionsChangedCallback) {
        ...
        this.interceptCheckbox = new Checkbox(container, {
            text: res.trendline.intercept,
            appendSpan: this.interceptInput.inputSpan
        }, "interceptCheckbox", function (checked) {
            if (checked) {
    ...
    function DataLabelsEditor(container, data, name, resources, optionsChangedCallback) {
        var self = this;
        self.datalabelsOptionsChanged = optionsChangedCallback;
        self.labelContains = new ParagraphText(container, resources.labelContainsText);
        this.showSeriesNameCheckbox = new Checkbox(container, { text: resources.showSeriesName }, "showSeriesName", function (checked) {
            var result = self.datalabelsOptionsChanged("showSeriesName", checked);
            if (result) {
                self.updateUI('showSeriesName', result);
            }
        });
        this.updateUI('showSeriesName', data.showSeriesName);
        this.showCategoryNameCheckbox = new Checkbox(container, { text: resources.showCategoryName }, "showCategoryName", function (checked) {
            var result = self.datalabelsOptionsChanged("showCategoryName", checked);
            if (result) {
                self.updateUI('showCategoryName', result);
            }
        });
        this.updateUI('showCategoryName', data.showCategoryName);
        this.showValueCheckbox = new Checkbox(container, { text: resources.showValue }, "showValue", function (checked) {
            var result = self.datalabelsOptionsChanged("showValue", checked);
            if (result) {
                self.updateUI('showValue', result);
            }
        });
        this.updateUI('showValue', data.showValue);
        if (data.showPercentage !== undefined) {
            this.showPercentCheckbox = new Checkbox(container, { text: resources.showPercentage }, "showPercentage", function (checked) {
                var result = self.datalabelsOptionsChanged("showPercentage", checked);
                if (result) {
                    self.updateUI('showPercentage', result);
                }
            });
            this.updateUI('showPercentage', data.showPercentage);
        }
        var separatorList = {};
        separatorList[comma] = ", ";
        separatorList[semicolon] = "; ";
        separatorList[period] = ". ";
        separatorList[newLine] = "\n";
        separatorList[space] = " ";
        this.showSeparatorDrop = new DropEditor(container, separatorListResvered[data.separator], "separator", resources.separator, Object.keys(separatorList), function (value) {
            var result = self.datalabelsOptionsChanged("separator", separatorList[value]);
            if (result) {
                self.updateUI('separator', separatorListResvered[result]);
            }
        });
        this.updateUI('separator', separatorListResvered[data.separator]);
        if (resources.positionItems) {
            self.labelPosition = new ParagraphText(container, resources.labelPositionText);
            this.positionRadioGroup = new RadioGroup(container, resources.positionItems, resources.position, function (value) {
                var result = self.datalabelsOptionsChanged('position', value);
                if (!isNullOrUndefined(result)) {
                    self.updateUI('position', datalabelPosition[result]);
                }
            });
            this.updateUI('position', datalabelPosition[data.position]);
        }
    }
    DataLabelsEditor.prototype.updateUI = function (type, value) {
        switch (type) {
            case 'showSeriesName':
                this.showSeriesNameCheckbox.setSelected(value);
                break;
            case 'showCategoryName':
                this.showCategoryNameCheckbox.setSelected(value);
                break;
            case 'showValue':
                this.showValueCheckbox.setSelected(value);
                break;
            case 'showPercentage':
                this.showPercentCheckbox.setSelected(value);
                break;
            case 'separator':
                this.showSeparatorDrop.updateUI(value);
                break;
            case 'position':
                this.positionRadioGroup.setSelectedItem(value);
                break;
        }
    };
    //#region LineEditor
    ...
    InputNumberEditor.prototype.addInputNumberEditor = function (title, container, option) {
        ...
        this.value = 0;
        var span = $('<span></span>').addClass('ui-spinner ui-widget ui-widget-content ui-corner-all').css({
            "width": option && option.inputWidth ? option && option.inputWidth : "85px",
            "float": "right",
            "margin-right": "3px",
            "height": "23px",
            "margin-top": "5px"
    ...
    InputNumberEditor.prototype.getValue = function () {
    ...
    function InputTextEditor(container, valueObj, valueChangedCallback, isShowAuto, isShowSelect, notShowInput) {
        this.valueChanged = valueChangedCallback;
        this.isShowAuto = isShowAuto || false;
        this.isShowSelect = isShowSelect || false;
        this.notShowInput = notShowInput || false;
        this.expectText = '';
        this.expectUnitScale = '';
        this.actualText = '';
        this.actualUnitScale = '';
        if (valueObj instanceof Object) {
            this.expectText = valueObj.text;
            this.expectUnitScale = valueObj.unitScale;
            this.addInputTextEditor(valueObj.resources, container);
        } else {
            this.addInputTextEditor(valueObj, container);
        }
    }
    ...
    InputTextEditor.prototype.addInputTextEditor = function (title, container) {
        ...
        var input, selector;
        if (self.isShowAuto) {
            self.autoDescLabel = $('<div></div>').addClass('input-auto-desc').text(designer.res.chartSliderPanel.auto);
            self.resetBtn = $('<div></div>').addClass('input-reset-btn').text(designer.res.chartSliderPanel.reset).click(function () {
                if (self.notShowInput) {
                    self.valueChanged(self.expectUnitScale, true);
                } else if (!self.isShowSelect) {
                    self.valueChanged(self.expectText);
                } else {
                    var value = {
                        unit: self.expectText,
                        unitScale: self.expectUnitScale
                    }
            ...
            if (!self.notShowInput) {
                input = $('<input />', { type: 'input' }).addClass('input-number-editor-auto').change(function (event) {
                    self.actualText = $(event.target).val();
                    var value = self.actualText;
                    if (self.isShowSelect) {
                        if (!isNullOrUndefined(self.actualUnitScale)) {
                            value = {
                                unit: isNaN(self.actualText) ? self.expectText : self.actualText,
                                unitScale: self.actualUnitScale
                            }
                        }
                    }
                    self.valueChanged(value, false);
            ...
            if (self.isShowSelect) {
                if (input) {
                    input.addClass('input-date-editor-auto');
                }
                selector = $("<select></select>").append(
                    "<option value= 0>" + designer.res.chartSliderPanel.dateScales.day +
                    "</option><option value= 1>" + designer.res.chartSliderPanel.dateScales.month +
                    "</option><option value= 2>" + designer.res.chartSliderPanel.dateScales.year + "</option>").addClass('date-axis-select-timeScale-editor').change(function (event) {
                        self.actualUnitScale = $(event.target).val();
                        var value = self.actualUnitScale;
                        if (!self.notShowInput) {
                            if (!isNullOrUndefined(self.actualText)) {
                                value = {
                                    unit: isNaN(self.actualText) ? self.expectText : self.actualText,
                                    unitScale: self.actualUnitScale
                                }
            ...
            input = $('<input />', { type: 'input' }).addClass('input-number-editor').change(function (event) {
                self.actualText = $(event.target).val();
                self.valueChanged(self.actualText);
            });
        }
        if (container) {
            ...
            self.domInput = $('<li></li>').append(label).append(input).append(selector).append(self.resetBtn).append(self.autoDescLabel).css({
                "margin": "1px",
                "list-style-type": "none",
                "line-height": "28px",
                "height": "28px"
            });
            container.append(self.domInput);
        } else {
    ...
    InputTextEditor.prototype.updateValue = function () {
        if (this.isShowAuto) {
            if ((!this.isShowSelect && (this.actualText === this.expectText)) || (this.isShowSelect && !this.notShowInput && this.actualText === this.expectText && this.actualUnitScale === this.expectUnitScale || (this.notShowInput && this.actualUnitScale === this.expectUnitScale))) {
            ...
            if (this.isShowSelect) {
                this.selector.val(this.actualUnitScale);
            }
        }
        if (!this.notShowInput) {
            this.inputSpan.val(this.actualText);
        }
    };
    InputTextEditor.prototype.hide = function () {
        this.domInput && this.domInput.hide();
    };
    InputTextEditor.prototype.show = function () {
        this.domInput && this.domInput.show();
    };
    ...
    ParagraphText.prototype.show = function () {
        this.dom.show();
    };
    ParagraphText.prototype.hide = function (value) {
        this.dom.hide();
    };
        ...
        switch (selectedElement) {
            case "series":
                ...
                if (chart && !isNullOrUndefined(i)) {
                    ...
                    } else if (property === 'borderLineType') {
                        option.border.lineType = value;
            ...
            case "axis":
                if (chart) {
                    ...
                    } else if (option.categoryType === 2 && property === 'majorUnit' || property === 'majorUnitScale') {
                        option.majorUnit = value.unit;
                        option.majorUnitScale = value.unitScale;
                    } else if (option.categoryType === 2 && property === 'minorUnit' || property === 'minorUnitScale') {
                        option.minorUnit = value.unit;
                        option.minorUnitScale = value.unitScale;
                    } else if (property === 'displayUnitVisible') {
                        if (!option.displayUnit) {
                            option.displayUnit = {};
                        }
                    ...
                    } else if (property === 'majorUnit') {
                        value = newData.majorUnit;
                    } else if (property === 'majorUnitScale') {
                        value = newData.majorUnitScale;
                    } else if (property === 'minorUnit') {
                        value = newData.minorUnit;
                    } else if (property === 'minorUnitScale') {
                        value = newData.minorUnitScale;
                    } else if (property === 'baseUnit') {
                        value = newData.baseUnit;
                    } else {
                        if (option.categoryType === 2 && (property === 'max' || property === 'min')) {
            ...
            case "legend":
                if (chart) {
                    ...
                    if (property === 'borderColor') {
                        ...
                        option[property] = value;
                    } else if (property === 'overlapping') {
                        option['showLegendWithoutOverlapping'] = value;
                    } else if (property === 'position') {//change legendPosition
                    ...
                    if (property === 'borderColor') {
                        ...
                        value = newData['showLegendWithoutOverlapping'];
                    } else {
                        value = newData[property];
                    }
                    names1 = namePath.split('.');
            ...
            case "dataLabels":
                ...
                if (chart && !isNullOrUndefined(i)) {
                    ...
                    if (property === 'position') {
                        var legendPositionMap = {};
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.bestFit.toLocaleLowerCase()] = 0;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.below.toLocaleLowerCase()] = 1;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.center.toLocaleLowerCase()] = 2;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.insideBase.toLocaleLowerCase()] = 3;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.insideEnd.toLocaleLowerCase()] = 4;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.left.toLocaleLowerCase()] = 5;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.outsideEnd.toLocaleLowerCase()] = 6;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.right.toLocaleLowerCase()] = 7;
                        legendPositionMap[designer.res.chartSliderPanel.dataLabels.above.toLocaleLowerCase()] = 8;
                        option.dataLabels[property] = legendPositionMap[value];
                    } else {
                        option.dataLabels[property] = value;
                    }
                    designer.actions.doAction("updateChartSeries", spread, {
                        chart: chart,
                        element: {
                    ...
                    newData = chart.series().get(i);
                    value = newData.dataLabels[property];
                    names1 = namePath.split('.');
                    for (j = 0; j < names1.length; j++) {
                        data = data[names1[j]];
                    }
    ...
    SliderPanelDataManager.prototype.getDatas = function () {
        ...
        for (var i in selectedData) { /* NOSONAR: Forin*/
            if (i !== "errorBar" && i !== "trendline" && i !== "dataLabels" && selectedData[i] instanceof Object) {
    ...
    SliderPanelDataManager.prototype.getSelectedData = function () {
        ...
        self._selectedSeries = self._selectedSeries ? self._selectedSeries : 0;
        if (item === "series") {
            return self._data["series"][self._selectedSeries];
        } else if (item === "errorBarX") {
            var chartTypeStr = chartHelper.getChartGroupString(self._chart.chartType());
            if (chartTypeStr === "BarGroup") {
        ...
        } else if (item === "dataLabels") {
            return self._data["series"][self._selectedSeries].dataLabels;
        }
        ...
        for (var i in data) {
            if (i === "series") {
                for (var j = 0; j < data[i].length; j++) {
                    ...
                    if (dataLabels && (chartType !== 57 && chartType !== 58)) {
                        selectedCollection.elements.push({
                            element: designer.res.chart.selectedOption.labelOptions,
                            text: designer.res.chart.selectedText.series + ' "' + series.name + '"' + designer.res.chart.selectedText.dataLabels,
                            item: "dataLabels",
                            seriesIndex: series.index
                        });
                    }
    ...
    SliderPanelDataManager.prototype.makeChartImp = function (chart, categories, dataPointIndex) {
        var data = {}, legend, chartTitle, chartArea, dataPoints;
        var chartType = this._chart.chartType();
        getSeriesData(chart, data, categories);
        getErrorBarData(chart, data, categories);
        getTrendlineData(chart, data, categories);
        if (chartType !== 57 && chartType !== 58) {
            getDataLabels(chart, data, categories);
        }
