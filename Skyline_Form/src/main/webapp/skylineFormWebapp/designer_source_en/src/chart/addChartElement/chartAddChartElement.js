(function () {
    ...
    AddChartElement.prototype._openPanel = function (domSelfKey) {
        ...
        if (domSelfKey) {
            ...
            if (domSelfKey === 'AxesOpenPanel') {
                ...
                chartElement = keyToNameObj[domSelfKey];
                seriesIndex = data && data.index;
                if (sSeries.length === 1) {
                    seriesIndex = 0; // Fix SJS-2148, it shouldn't show the series selections dialog if only have one series.
                }
                if (seriesIndex === undefined) {
                    var errorBarsDialog = new designer.chartErrorBarsDialog(sCollection, sSeries, 1, names, chartSliderPanel, this._activeChart, chartElement, selectedItem);
                ...
                if (seriesIndex === undefined) {
                    var trendlineDialog = new designer.chartTrendlineDialog(sCollection, sSeries, 1, names, this._activeChart, chartSliderPanel, chartElement, selectedItem);
            ...
            chartSliderPanel.initChartSliderPanel(this._activeChart, chartElement, selectedItem, seriesIndex, names);
        }
    };
        ...
        $subMenuItemAll.click(function (e) {
            ...
            switch (parentKey) {
                ...
                case 'dataLabels':
                    ...
                    var datalabelSeries = seriesCollection.get();
                    var dataLabels = self._activeChart.dataLabels();
                    var i = 0, index, series;
    ...
    AddChartElement.prototype._needShowSubMenuItemWithObjKeyAndSubMenuKey = function (objKey, subMenuKey) {
        ...
        if (this._chartGroupNameArray.length > 0) {
            ...
            var isStackChart = chartHelper.isStackedChart(this._activeChart.chartType());
            ...
            if (isStackChart && objKey === 'dataLabels' && subMenuKey === 'dataLabelsOutsideEnd') {
                return false;
            }
