...
var formulaToRange = GC.Spread.Sheets.CalcEngine.formulaToRange;
    ...
    var SelectChartDialog = (function (_super) {
        ...
        SelectChartDialog.prototype._createChartOptions = function () {
            ...
            var axes, dataLabels, chartArea, legend, dataOrientation, title, ignoreHidden;
            if (self._chart) {
                ...
                ignoreHidden = self._chart.ignoreHidden();
            }
    ...
    var chartSelectDataDialog = (function (_super) {
        ...
        function ChartSelectDataDialog() {
            ...
            this.series = null;
            this.hiddenAndEmptyCellsOptions = null;
        }
                        ...
                        click: function () {
                            ...
                            if (isChanged) {
                                designer.actions.doAction("updateChartAllSeries", designer.wrapper.spread, {
                                    ...
                                    hiddenAndEmptyOptions: self.hiddenAndEmptyCellsOptions,
                                    chart: self._activeChart
                                });
                            }
                            var hiddenAndEmptyCellsOptions = self.hiddenAndEmptyCellsOptions;
                            if (hiddenAndEmptyCellsOptions) {
                                designer.actions.doAction("changeChartElement", designer.wrapper.spread, {
                                    displayBlanksAs: hiddenAndEmptyCellsOptions.index,
                                    displayNaAsBlank: hiddenAndEmptyCellsOptions.displayNaAsBlank,
                                    ignoreHidden: hiddenAndEmptyCellsOptions.showHiddenRowsAndColumnsVal
                                });
                            }
                        }
                    },
        ...
        ChartSelectDataDialog.prototype._init = function () {
            ...
            $(".hidden-empty-button").on('click', function () {
                var chartGroupString = chartHelper.getChartGroupString(self._activeChart.chartType());
                ...
                if (!self._hiddenEmpty) {
                    self._hiddenEmpty = new designer.chartHiddenEmptyCellDialog(function (index, displayNaAsBlank, showHiddenRowsAndColumnsVal) {
                        self.hiddenAndEmptyCellsOptions = {
                            index: index,
                            displayNaAsBlank: displayNaAsBlank,
                            showHiddenRowsAndColumnsVal: !showHiddenRowsAndColumnsVal,
                        }
                    });
                }
        ...
        this._chartDataOrientation =  args[0].getDataOrientation();
        ...
        ChartSelectDataDialog.prototype._addSeriesModel = function (item) {
            ...
            if (series.yValues) {
                ...
                if (beforeRange && afterRange && beforeRange.rowCount === afterRange.rowCount && beforeRange.colCount === afterRange.colCount) {
            ...
            for (var i = 0; i < this._middleChartSeriesModel.length; i++) {
                ...
                if (series.name) {
                    ...
                    if (range) {
                        ...
                        var row = range.row, col = range.col, rowCount = range.rowCount, colCount = range.colCount, seriesNames = [];
                        for (var j = row; j < row + rowCount; j++) {
                            for (var k = col; k < col + colCount; k++) {
                                var cell = sheet.getCell(j, k);
                                seriesNames.push(cell.text());
                            }
                        }
                        seriesName = seriesNames.join(' ');
                    } else {
        ...
        ChartSelectDataDialog.prototype._isCellVisible = function (sheet, row, col) {
            return sheet && sheet.getRowVisible(row, 3, false) && sheet.getRowHeight(row) > 0 && sheet.getColumnVisible(col, 3) && sheet.getColumnWidth(col) > 0;
        }
        ...
        ChartSelectDataDialog.prototype._getRangeByFormula = function (formula) {
            if (formula) {
                ...
                var rangeInfo = sheet.getReferenceRange(formula);
                return rangeInfo && rangeInfo.range;
        ...
        ChartSelectDataDialog.prototype._getValuesByRange = function (range, sheet) {
            ...
            var ignoreHidden = this._activeChart && this._activeChart.ignoreHidden();
            if (range) {
                if ((chartGroupString === chartHelper.chartTypeDict["57"].chartGroup || chartGroupString === chartHelper.chartTypeDict["58"].chartGroup)) {
                    ...
                    if (!isNullOrUndefined(formualX) && !isNullOrUndefined(formualY)) {
                        ...
                        var catDataArray = ignoreHidden ? this._getVisibleDataArray(sheet, rangeX.row, rangeX.col, rangeX.rowCount, rangeX.colCount) : sheet.getArray(rangeX.row, rangeX.col, rangeX.rowCount, rangeX.colCount);
                        var valueArray = ignoreHidden ? this._getVisibleDataArray(sheet, rangeY.row, rangeY.col, rangeY.rowCount, rangeY.colCount) : sheet.getArray(rangeY.row, rangeY.col, rangeY.rowCount, rangeY.colCount);
                ...
                var orientation = this._chartDataOrientation;
                    if (orientation === 0) {
                        for (var i = 0; i < range.colCount; i++) {
                            if (!ignoreHidden || this._isCellVisible(sheet, range.row, range.col + i)) {
                                var textArr = [];
                                for (var j = range.row; j < range.rowCount + range.row; j++) {
                                    textArr.push(sheet.getCell(j, range.col + i).text());
                                }
                                valuesArray.push(textArr.join(' '));
                            }
                        }
                    } else if (orientation === 1) {
                        for (var j = 0; j < range.rowCount; j++) {
                            if (!ignoreHidden || this._isCellVisible(sheet, range.row + j, range.col)) {
                                var textArr = [];
                                for (var k = range.col; k < range.col + range.colCount; k++) {
                                    textArr.push(sheet.getCell(range.row + j, k).text());
                                }
                                valuesArray.push(textArr.join(' '));
                            }
                        }
        ...
        ChartSelectDataDialog.prototype._getVisibleDataArray = function (sheet, row, column, rowCount, columnCount) {
            var array = [];
            var rc = sheet.getRowCount(), cc = sheet.getColumnCount();
            if (0 <= row && row < rc && 0 <= column && column < cc) {
                ...
                for (var i = 0; i < rowCount; i++) {
                    ...
                    for (var j = 0; j < columnCount; j++) {
                        if (this._isCellVisible(sheet, row + i, column + j)) {
                            array[i][j] = sheet.getValue(row + i, column + j);
                        }
                    }
                }
            }
        ...
        ChartSelectDataDialog.prototype._addEventListener = function () {
            ...
            self._chartDataOrientation = self._chartDataOrientation === 0 ? 1 : 0;
        ...
        ChartSelectDataDialog.prototype._rangeSelectCallBack = function (newFormula) {
            ...
            newFormula = this._changeAllFormulaToNormalFormula(newFormula);
        ...
        ChartSelectDataDialog.prototype._addCallBack = function (addResult) {
            if (addResult) {
                ...
                for (var key in addResult) {
                    result[key] = this._changeAllFormulaToNormalFormula(addResult[key]);
                }
        ...
        ChartSelectDataDialog.prototype._editCategoryCallBack = function (categoryFormula) {
            if (categoryFormula && categoryFormula !== '=') {
                var self = this;
                categoryFormula = this._changeAllFormulaToNormalFormula(categoryFormula);
                // var newSheet = chartHelper.getSheetFromFormula(categoryFormula);
                // var range = formulaToRange(newSheet, categoryFormula);
                var range = this._getRangeByFormula(categoryFormula);
                var oldSeries = self._middleChartSeriesModel[self._selectedSeries.data('seriesIndex')];
                if (categoryFormula === oldSeries.xValues) {
                    return;
                }
                // var oldSheet = chartHelper.getSheetFromFormula(oldSeries.xValues);
                var oldXValueRange = this._getRangeByFormula(oldSeries.xValues);// formulaToRange(oldSheet, oldSeries.xValues);
                var oldYValuesRange = this._getRangeByFormula(oldSeries.yValues); // formulaToRange(oldSheet, oldSeries.yValues);
                var isOldRangeAndEqual = oldXValueRange && range && range.rowCount === oldXValueRange.rowCount && range.colCount === oldXValueRange.colCount;
        ...
        ChartSelectDataDialog.prototype._editCallBack = function (editResult) {
            if (this._selectedSeries) {
                ...
                for (var key in editResult) {
                    result[key] = this._changeAllFormulaToNormalFormula(editResult[key]);
                }
        ...
        ChartSelectDataDialog.prototype._changeAllFormulaToNormalFormula = function (beforeFormula) {
            var activeSheet = designer.wrapper.spread.getActiveSheet();
            var expr = {};
