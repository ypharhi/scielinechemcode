(function () {
    ...
    function reset(init) {
        ...
        try {
            ...
            wrapper.spread.setSheetCount(1);
            ...
            onReadySpreadJS_(wrapper.spread);//COMPLY_SPREADSHEET
        } catch (ex) {
        ...
        wrapper.spread.bind(Sheets.Events.TableRowsChanged, function (event, args) {
            if (args.propertyName === 'tableInsertRows') {
                // update sparkline when table insert rows.
        ...
        wrapper.spread.bind(Sheets.Events.TableColumnsChanged, function (event, args) {
            if (args.propertyName === 'tableInsertColumns') {
                // update sparkline when table insert columns.
    ...
    function setFocusToSpread() {
        if (wrapper.spread.getActiveSheet()) {
            wrapper.spread.focus(false); //comply change from false for entering value to a1 cell init
        }
    }
