(function () {
    ...
    var CUT = "cut",
        ...
        TABLE = 'table',
        INSERT_COPY_CELLS = "insertCopiedCells", INSERT_CUT_CELLS = "insertCutCells",
        SPREAD_COMMAND_PREFIX = "gc.spread.contextMenu.",
        SET_HYPERLINK = 'setHyperlink',
        LINK = 'link',
        EDIT_HYPERLINK = 'editHyperlink',
        OPEN_URL = 'openUrl',
        OPEN_HYPERLINK = 'openHyperlink',
        REMOVE_HYPERLINK = 'removeHyperlink',
        REMOVE_HYPERLINKS = 'removeHyperlinks',
        OPEN_HYPERLINK_DIALOG = 'openHyperlinkDialog';
    ...
    function initDesignerCommands(commandManager) {
        ...
        commandManager.register(MENUITEM_NAME_PREFIX + SHOW_FORMULAS, commands_setShowFormulas, 192 /* ` */, true, false, false, false);
        commandManager.register(OPEN_HYPERLINK_DIALOG, Commands[OPEN_HYPERLINK_DIALOG], 75 /* k */, ctrl, false, false, meta);
        commandManager.register(MENUITEM_NAME_PREFIX + LINK, Commands[LINK], null, false, false, false, false);
        commandManager.register(MENUITEM_NAME_PREFIX + EDIT_HYPERLINK, Commands[EDIT_HYPERLINK], null, false, false, false, false);
        commandManager.register(MENUITEM_NAME_PREFIX + OPEN_HYPERLINK, Commands[OPEN_HYPERLINK], null, false, false, false, false);
        commandManager.register(MENUITEM_NAME_PREFIX + REMOVE_HYPERLINK, Commands[REMOVE_HYPERLINK], null, false, false, false, false);
        commandManager.register(MENUITEM_NAME_PREFIX + REMOVE_HYPERLINKS, Commands[REMOVE_HYPERLINKS], null, false, false, false, false);
    }
    ...
    var DesignerActionBase = (function (_super) {
        ...
        DesignerActionBase.prototype.execInSelections = function (sheet, func) {
            ...
            var sheetRowCount = sheet.getRowCount(), sheetColCount = sheet.getColumnCount();
            for (var k = 0; k < selections.length; k++) {
                ...
                if ((col === -1 && row === -1) || (colCount === sheetColCount && rowCount === sheetRowCount)) {
                    func(sheet, -1, -1);
                } else if (row === -1 || rowCount === sheetRowCount) {
                    for (i = 0; i < colCount; i++) {
                        func(sheet, -1, col + i);
                    }
                } else if (col === -1 || colCount === sheetColCount) {
    ...
    //#endregion
    var SetShowFormulasAction = (function (_super) {
        designer.extends(SetShowFormulasAction, _super);
        function SetShowFormulasAction(spread, options) {
            _super.call(this, spread, options);
        }
        SetShowFormulasAction.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            if (options.sheetName) { //From the short cut, toggle the showFormulas.
                var sheet = context.getSheetFromName(options.sheetName);
                sheet.options.showFormulas = !sheet.options.showFormulas;
            } else { //from the ribbon
                options.sheet.options.showFormulas = options.value;
            }
            self.updateRibbon();
            return true;
        };
        return SetShowFormulasAction;
    })(DesignerActionBase);
    var commands_setShowFormulas = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new SetShowFormulasAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function setShowFormulas(spread, paramters) {
        runCommand(spread, SHOW_FORMULAS, commands_setShowFormulas, {
            value: paramters.options,
            sheet: paramters.sheet
        });
    }
    spreadActions.setShowFormulas = setShowFormulas;
    ...
    var InsertRightCellsAction = (function (_super) {
        ...
        InsertRightCellsAction.prototype.canInsertRight = function (sortedRanges) {
            ...
            for (var i = 0; i < sortedRanges.length; i++) {
                ...
                var affectRange = new Range(row, col, endRow - row, sheet.getColumnCount());
    ...
    var InsertDownCellsAction = (function (_super) {
        ...
        InsertDownCellsAction.prototype.canInsertDown = function (sortedRanges) {
            ...
            for (var i = 0; i < sortedRanges.length; i++) {
                ...
                var endRow = range.row + range.rowCount;
                var endCol = range.col + range.colCount;
                var affectRange = new Range(row, col, sheet.getRowCount(), endCol - col);
                var tableAffectRet = canShiftCell(sheet, affectRange);
                if (tableAffectRet) {
                    return tableAffectRet;
                }
    ...
    var ClearAllAction = (function (_super) {
        ...
        ClearAllAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            for (var i = 0; i < selections.length; i++) {
                var selection = selections[i];
                self.sheet().clear(selection.row, selection.col, selection.rowCount, selection.colCount, 3 /* viewport */, 32 /* Axis */ | 64 /* BindingPath */ | 1 /* Data */ | 16 /* Sparkline */ | 2 /* Style */ | 4 /* Comment */ | 256 /* Hyperlink */);
    ...
    var ReApplyRowFilterAction = (function (_super) {
        ...
        ReApplyRowFilterAction.prototype.canExecute = function () {
            var self = this, sheet = self._sheet,
                ...
                table = sheet.tables.find(activeRowIndex, activeColIndex);
            if ((sheet.rowFilter() && sheet.rowFilter().isFiltered()) || (table && table.rowFilter() && table.rowFilter().isFiltered())) {
                return true;
            }
            return false;
        };
    ...
    var SetAutoFormulaAction = (function (_super) {
        ...
        SetAutoFormulaAction.prototype.cellRangeInflate = function (spans, cellRange) {
        ...
        SetAutoFormulaAction.prototype.calculateValidRange = function (row, column, rowCount, columnCount) {
            var r = row;
            var c = column;
            var rc = rowCount;
            var cc = columnCount;
            if (r !== -1 && c !== -1) {
                var spans = this._sheet.getSpans();
        ...
        SetAutoFormulaAction.prototype.calculateRange = function (startWithLeft, range) {
            var row, col, rowCount = 1, colCount = 1, sheet = this._sheet, pureNull = false;
            if (startWithLeft === false) {
                var includeNumber = false;
                for (var i = range.row - 1; i >= 0; i--) {
                    var value = sheet.getValue(i, range.col)/*, formula = sheet.getFormula(i, range.col)*/;
            ...
            if (!pureNull) {
                ...
                for (var i = range.col - 1; i >= 0; i--) {
                    var value = sheet.getValue(range.row, i)/*, formula = sheet.getFormula(range.row, i)*/;
                    if (this.valueIsNumber(value)) {
                        includeNumber = true;
                        pureNull = true;
                        if (i === 0) {
        ...
        SetAutoFormulaAction.prototype.valueIsNumber = function (value) {
            if (typeof value === 'number') {
                return true;
            }
            return false;
        };
        SetAutoFormulaAction.prototype.getStartRangeDirection = function (range, sheet, leftCell, topCell) {
            var leftRangeHaveNumberValue = false, topRangeFirstValueFormula = false, startWithLeft = false;
            ...
            if ((this.valueIsNumber(leftCell) && !this.valueIsNumber(topCell)) || (leftRangeHaveNumberValue && topRangeFirstValueFormula)) {
                startWithLeft = true;
            }
            return startWithLeft;
        };
        SetAutoFormulaAction.prototype.getFormulaRange = function (range) {
            var self = this;
            var row, col, rowCount = 1, colCount = 1, sheet = self._sheet,
                leftCell = sheet.getValue(range.row, range.col - 1),
                topCell = sheet.getValue(range.row - 1, range.col), startWithLeft = false;
            startWithLeft = this.getStartRangeDirection(range, sheet, leftCell, topCell);
            var calculateRange = this.calculateRange(startWithLeft, range);
            row = calculateRange.row, col = calculateRange.col, rowCount = calculateRange.rowCount, colCount = calculateRange.colCount;
            var formulaRange = {};
            if (col === undefined && row === undefined) {
                formulaRange = range;
            } else if (col === undefined) {
                var validRange = this.calculateValidRange(row, range.col, range.row - row, colCount);
                formulaRange.row = validRange.row;
                formulaRange.col = validRange.col;
                formulaRange.colCount = validRange.colCount;
                formulaRange.rowCount = validRange.rowCount;
            } else if (row === undefined) {
                var validRange = this.calculateValidRange(range.row, col, rowCount, range.col - col);
                formulaRange.row = validRange.row;
                formulaRange.col = validRange.col;
                formulaRange.colCount = validRange.colCount;
                formulaRange.rowCount = validRange.rowCount;
        ...
        SetAutoFormulaAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            if (getSelectionTypeWithSelections(self.getSelections()) === 3 /* OnlyCells */) {
                ...
                for (var i = 0; i < selections.length; i++) {
                    ...
                    if (selection.row + selection.rowCount === self._sheet.getRowCount() && (!(selection.colCount === 1 && selection.rowCount === 1))) {
                        continue;
                    } else {
        ...
        SetAutoFormulaAction.prototype.setWholeRowFormula = function (setNextCell, lastRow, range, formulaName) {
            var setFormulaCellRow = setNextCell ? lastRow : lastRow + 1, self = this;
            for (var i = range.col; i < range.col + range.colCount; i++) {
                ...
                var formula = rangeToFormula(formulaRange, setFormulaCellRow, i, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                formula = "=" + formulaName + "(" + formula + ")";
                self._sheet.setFormula(setFormulaCellRow, i, formula);
            }
        }

        SetAutoFormulaAction.prototype.setWholeColFormula = function (lastCol, range, formulaName) {
            var self = this;
            for (var i = range.row; i < range.row + range.rowCount; i++) {
                ...
                var formula = rangeToFormula(formulaRange, i, lastCol - 1, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                formula = "=" + formulaName + "(" + formula + ")";
                self._sheet.setFormula(i, lastCol, formula);
            }
        }

        SetAutoFormulaAction.prototype._setMulitRowColRangeFormula = function (range, formulaName) {
            var self = this, lastRow = range.row + range.rowCount - 1,
                lastCol = range.col + range.colCount - 1, sheet = self._sheet, setNextCell = true, setRightCell = true;
            for (var i = range.col; i < range.col + range.colCount; i++) {
        ...
        SetAutoFormulaAction.prototype._noNeedSetFormula = function (row, col) {
            var sheet = this._sheet, style = sheet.getStyle(row, col);
        ...
        SetAutoFormulaAction.prototype._setFormulaForSingleRange = function (range, formulaName) {
            var self = this, sheet = self._sheet;
            // now, all I care about is that a cell of type number is going to do AutoSum,
            // and in Excel, we're going to do a time type, like 2020/03/02, and then we're going to do number on the formatter of the cell
            // and Excel is going to do AutoSum, but we're not going to do it.We're only dealing with cells of type number.

            //There are special cases, for example, when Excel does antosum,
            // sometimes it includes a value with a Formula in it, and sometimes it doesn't.I think it may be a bug in Excel.
            // Now I have implemented a normal case, and I will follow up these strange points in time, if there is any need of users.

            //Please check the specific strategy:
            //https://wiki.grapecity.io/display/SJS/Sheets+Designer+Auto+Functions+to+Match+Excel

            //if range is a span
            var span = sheet.getSpan(range.row, range.col), rangeIsSpan = false;
            if (span && span.equals(range)) {
                rangeIsSpan = true;
            }
            //A single cell
            if ((range.colCount === 1 && range.rowCount === 1) || rangeIsSpan) {
                if (self._noNeedSetFormula(range.row, range.col)) {
                    return;
                }
                var formulaRange = self.getFormulaRange(range);
                var formulaString, selectRange = true;;
                if (formulaRange.rowCount === 1 && formulaRange.colCount === 1 && formulaRange.row === range.row && formulaRange.col === range.col) {
                    formulaString = "";
                    selectRange = false;
                } else {
                    formulaString = rangeToFormula(formulaRange, range.row, range.col, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                }
                var formula = "=" + formulaName + "(" + formulaString + ")";
                sheet.inputFormulaAndSelectFormulaRange(range.row, range.col, selectRange, formula);
            } else if (range.colCount === 1) {
                var row = range.row + range.rowCount - 1, col = range.col;
                if (!sheet.getText(row, col)) {
                    ...
                    var formulaString = rangeToFormula(formulaRange, row, col, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                    var formula = "=" + formulaName + "(" + formulaString + ")";
                    sheet.setFormula( row, col, formula);
                } else if (row + 1 < sheet.getRowCount()) {
                    var formulaString = rangeToFormula(range, row + 1, col, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                    var formula = "=" + formulaName + "(" + formulaString + ")";
                    sheet.setFormula( row + 1, col, formula);
                }
            } else if (range.rowCount === 1) {
                var row = range.row, col = range.col + range.colCount - 1;
                if (!sheet.getText(row, col)) {
                    var formulaRange = {
                        row: range.row,
                        col: range.col,
                    ...
                    var formula = "=" + formulaName + "(" + formulaString + ")";
                    sheet.setFormula( row, col, formula);
                } else if (col + 1 < sheet.getColumnCount()) {
                    var formulaString = rangeToFormula(range, row , col + 1, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                    var formula = "=" + formulaName + "(" + formulaString + ")";
                    sheet.setFormula( row, col + 1, formula);
                }
            } else {
                self._setMulitRowColRangeFormula(range, formulaName);
            }
        };
    ...
    var SetTableAllowAutoExpand = (function (_super) {
        designer.extends(SetTableAllowAutoExpand, _super);
        function SetTableAllowAutoExpand(spread, options) {
            _super.call(this, spread, options);
        }
        SetTableAllowAutoExpand.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            self._value = options.value;
            var activeTable = self._sheet.tables.findByName(options.tableName);
            if (activeTable instanceof Sheets.Tables.Table) {
                activeTable.allowAutoExpand(self._value);
            }
        };
        return SetTableAllowAutoExpand;
    })(DesignerActionBase);
    var commands_setTableAllowAutoExpand = {
        canUndo: true,
        execute: function (context, options, isUndo) {
            var cmd = new SetTableAllowAutoExpand(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function setTableAllowAutoExpand(spread, paramters) {
        var sheet = paramters.sheet;
        var activeTableName = sheet.tables.find(sheet.getActiveRowIndex(), sheet.getActiveColumnIndex()).name();
        runCommand(spread, "setTableAllowAutoExpand", commands_setTableAllowAutoExpand, {
            value: paramters.options,
            tableName: activeTableName,
            sheetName: paramters.sheetName
        });
    }
    spreadActions.setTableAllowAutoExpand = setTableAllowAutoExpand;
    ...
    var SetHyperlinkAction = (function (_super) {
        designer.extends(SetHyperlinkAction, _super);
        function SetHyperlinkAction(spread, options) {
            _super.call(this, spread, options);
        }
        SetHyperlinkAction.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            var hyperlink = options.options.hyperlink;
            var text = options.options.text;
            self.execInSelections(self._sheet, function (sheet, row, column) {
                var colCount = sheet.getColumnCount(), rowCount = sheet.getRowCount();
                var r, c;
                if (row === -1 && column === -1) {
                    for (r = 0; r < rowCount; r++) {
                        for (c = 0; c < colCount; c++) {
                            sheet.setHyperlink(r, c, hyperlink);
                            sheet.setValue(r, c, text);
                        }
                    }
                } else if (row === -1) {
                    for (r = 0; r < rowCount; r++) {
                        sheet.setHyperlink(r, column, hyperlink);
                        sheet.setValue(r, column, text);
                    }
                } else if (column === -1) {
                    for (c = 0; c < colCount; c++) {
                        sheet.setHyperlink(row, c, hyperlink);
                        sheet.setValue(row, c, text);
                    }
                } else {
                    sheet.setHyperlink(row, column, hyperlink);
    ...
    Commands[SET_HYPERLINK] = {
        canUndo: true,
        execute: function (context, options, isUndo) {
            var cmd = new SetHyperlinkAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function setHyperlinkAction(spread, options) {
        runCommand(spread, SET_HYPERLINK, Commands[SET_HYPERLINK], options);
    }
    spreadActions.setHyperlinkAction = setHyperlinkAction;
    var OpenHyperlinkDialogAction = (function (_super) {
        designer.extends(OpenHyperlinkDialogAction, _super);
        function OpenHyperlinkDialogAction(spread, options) {
            _super.call(this, spread, options);
        }
        OpenHyperlinkDialogAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
        ...
        return OpenHyperlinkDialogAction;
    })(DesignerActionBase);
    Commands[OPEN_HYPERLINK_DIALOG] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new OpenHyperlinkDialogAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function openHyperlinkDialog(spread, options) {
        runCommand(spread, OPEN_HYPERLINK_DIALOG, Commands[OPEN_HYPERLINK_DIALOG], options);
    }
    spreadActions.openHyperlinkDialog = openHyperlinkDialog;
    var LinkAction = (function (_super) {
        designer.extends(LinkAction, _super);
        function LinkAction(spread, options) {
            _super.call(this, spread, options);
        }
        LinkAction.prototype.executeImp = function (context, options, isUndo) {
            openHyperlinkDialog(context, options, isUndo);
        };
        return LinkAction;
    })(DesignerActionBase);
    Commands[LINK] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new LinkAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function link(spread, options) {
        runCommand(spread, LINK, Commands[LINK], options);
    }
    spreadActions.link = link;
    var EditHyperlink = (function (_super) {
        designer.extends(EditHyperlink, _super);
        function EditHyperlink(spread, options) {
            _super.call(this, spread, options);
        }
        EditHyperlink.prototype.executeImp = function (context, options, isUndo) {
            openHyperlinkDialog(context, options, isUndo);
        };
        return EditHyperlink;
    })(DesignerActionBase);
    Commands[EDIT_HYPERLINK] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new EditHyperlink(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function editHyperlink(spread, options) {
        runCommand(spread, EDIT_HYPERLINK, Commands[EDIT_HYPERLINK], options);
    }
    spreadActions.editHyperlink = editHyperlink;
    var OpenHyperlinkAction = (function (_super) {
        designer.extends(OpenHyperlinkAction, _super);
        function OpenHyperlinkAction(spread, options) {
            _super.call(this, spread, options);
        }
        OpenHyperlinkAction.prototype.executeImp = function (context, options, isUndo) {
            var spread = this._spread;
            var self = this;
            self.execInSelections(self._sheet, function (sheet, row, column) {
                var command = {};
                command.cmd = OPEN_URL;
                command.sheetName = sheet.name();
                command.url = sheet.getHyperlink(row, column, 3 /* viewport */).url;
                spread.commandManager().execute(command);
            });
        };
        return OpenHyperlinkAction;
    })(DesignerActionBase);
    Commands[OPEN_HYPERLINK] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new OpenHyperlinkAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function openUrl(spread, options) {
        runCommand(spread, OPEN_HYPERLINK, Commands[OPEN_HYPERLINK], options);
    }
    spreadActions.openUrl = openUrl;
    var RemoveHyperlinkAction = (function (_super) {
        designer.extends(RemoveHyperlinkAction, _super);
        function RemoveHyperlinkAction(spread, options) {
            _super.call(this, spread, options);
        }
        RemoveHyperlinkAction.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            self.execInSelections(self._sheet, function (sheet, row, column) {
                sheet.setHyperlink(row, column, null);
            });
        };
        return RemoveHyperlinkAction;
    })(DesignerActionBase);
    Commands[REMOVE_HYPERLINK] = {
        canUndo: true,
        execute: function (context, options, isUndo) {
            var cmd = new RemoveHyperlinkAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function removeHyperlink(spread, options) {
        runCommand(spread, REMOVE_HYPERLINK, Commands[REMOVE_HYPERLINK], options);
    }
    spreadActions.removeHyperlink = removeHyperlink;
    var RemoveHyperlinksAction = (function (_super) {
        designer.extends(RemoveHyperlinksAction, _super);
        function RemoveHyperlinksAction(spread, options) {
            _super.call(this, spread, options);
        }
        RemoveHyperlinksAction.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            self.execInSelections(self._sheet, function (sheet, row, column) {
                var colCount = sheet.getColumnCount(), rowCount = sheet.getRowCount();
                var r, c;
                if (row === -1 && column === -1) {
                    for (r = 0; r < rowCount; r++) {
                        for (c = 0; c < colCount; c++) {
                            sheet.setHyperlink(r, c, null);
                        }
                    }
                } else if (row === -1) {
                    for (r = 0; r < rowCount; r++) {
                        sheet.setHyperlink(r, column, null);
                    }
                } else if (column === -1) {
                    for (c = 0; c < colCount; c++) {
                        sheet.setHyperlink(row, c, null);
                    }
                } else {
                    sheet.setHyperlink(row, column, null);
                }
        ...
        return RemoveHyperlinksAction;
    })(DesignerActionBase);
    Commands[REMOVE_HYPERLINKS] = {
        canUndo: true,
        execute: function (context, options, isUndo) {
            var cmd = new RemoveHyperlinksAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
    function removeHyperlinks(spread, options) {
        runCommand(spread, REMOVE_HYPERLINKS, Commands[REMOVE_HYPERLINKS], options);
    }
    spreadActions.removeHyperlinks = removeHyperlinks;
    //#region comments
    ...
    var moveChartAction = (function (_super) {
        ...
        MoveChartAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            spread.setActiveSheet(options.targetSheetName);
            designer.wrapper.spread.undoManager().clear();
        };
    ...
    var changeShapeSizeAndPropertiesAction = (function (_super) {
        ...
        changeShapeSizeAndPropertiesAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            var allowResize = options.allowResize;
            var allowRotate = options.allowRotate;
            var allowMove = options.allowMove;
            var showHandle = options.showHandle;
            ...
            for (var i = 0; i < selectedShapeArray.length; i++) {
                ...
                if (allowResize !== undefined) {
                    this.updateProp(tempShape, "allowResize", allowResize);
                }
                if (allowRotate !== undefined) {
                    this.updateProp(tempShape, "allowRotate", allowRotate);
                }
                if (allowMove !== undefined) {
                    this.updateProp(tempShape, "allowMove", allowMove);
                }
                if (showHandle !== undefined) {
                    this.updateProp(tempShape, "showHandle", showHandle);
                }
    ...
    function changeShapeSizeAndProperties(spread, paramters) {
        runCommand(spread, "changeShapeSizeAndProperties", commands_changeShapeSizeAndProperties, {
            ...
            allowResize: paramters.options.allowResize,
            allowRotate: paramters.options.allowRotate,
            allowMove: paramters.options.allowMove,
            showHandle: paramters.options.showHandle,
            sheetName: paramters.sheetName,
            ignoreEvent: paramters.options.ignoreEvent,
