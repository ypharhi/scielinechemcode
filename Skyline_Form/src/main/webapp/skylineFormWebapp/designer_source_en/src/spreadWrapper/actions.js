(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    try {
                        ...
                        adjustConditionalFormatValue(data);
                ...
                error: function () {
                    return designer.MessageBox.show(designer.res.requestTemplateFail, designer.res.title, 3 /* error */);
                }
            });
        };
    ...
    function adjustConditionalFormatValue(data) {
        var adjustCFValue = function (val) {
            if (typeof val === "string") {
                val = $.trim(val);
                if (val[0] !== '=') {
                    var temVal = val.replace(new RegExp('"', 'gi', '""'));
                    val = '="' + temVal + '"';
                }
            }
            return val;
        ...
        for (var name in sheets) {
            ...
            if (sheet.conditionalFormats) {
                sheet.conditionalFormats.rules.forEach(function (rule) {
                    if (rule.ruleType === 1) {
                        if (rule.value1 !== void 0) {
                            rule.value1 = adjustCFValue(rule.value1);
                        }
                        if (rule.value2 !== void 0) {
                            rule.value2 = adjustCFValue(rule.value2);
                        }
                    }
                });
            }
