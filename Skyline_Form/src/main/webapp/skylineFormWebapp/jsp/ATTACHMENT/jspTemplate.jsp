...
<script>
	$(document).ready(function() { 
	    ...
	    if (window.self === window.top){    	
	    	 if($('#backUrl').val() != ""){	    	
	    		 $('#close_back').attr('onclick','confirmWithOutSave(function(){$(\'#doBackForm\').submit();})');	    		 
	    	 }
	    	 else{
	    	...
	    	$('#close_back').attr('onclick', 'confirmWithOutSave(function(){parent.$(\'#prevDialog\').dialog(\'close\');});');
					...
					<tr>
						<td class="ui-widget-content ui-corner-all innerTD" colspan="10">
							<table class="displayOnLoad" width="100%" style="display:none;">
								<tr>
									<td class="innerTD">
									 <!--begin --><div id = "popupTemplate" style="height: calc(100vh - 115px);overflow-y: auto;">
									@body@
									<!--end --></div>
									</td>
								</tr>	
						...
						<td class="submit-button-row">
							...
							<button  type="button" id="save_" perm_attr="cu" class="button" onclick="doSave('@afterSave@')"><spring:message code="Save" text="Save" /></button>
							<button id="close_back" type="button" class="button" style="margin-left: 15px;" onclick="parent.$('#prevDialog').dialog('close');"><spring:message code="Close" text="Close" /></button>
						</td>									
					</tr>								
				</table>
			</td>
