...
function add_arrowDDL(domId){
    ...
    +'<b class="arrows-search" style="height:100%;background: url(../skylineFormWebapp/js/chosen-sprite.png) no-repeat 0 5px;"></b>'
    +'</div>';
	...
	if(o[0].hasAttribute("saveValueAsJSON"))
	{
		...
		else
		{
			...
			if(typeof changeType !== 'undefined' && changeType !=null && changeType == 2 && retVal != null && retVal == 'ALL') {  // YP 13022019 - prevent FirstOption ALL value to enetr into the DB (critical in id val ddl) on save (changeType = 2 in ajax dataflow changeType = 1 we need it ALL)
				if($('#formCode').val()!='PermissionScheme' && $('#formCode').val()!='ExperimentReport'){//TODO: may the REMOVE_AND_SAVE_ALL configuration replace this condition
					retVal = "";
				}
				//retVal = "";
			}
