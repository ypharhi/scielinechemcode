...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo, drop, invalidateRows )
{
	...
	{
//		<ColReorder compatibility changes> ab/082020 patch for 'ColReorder' library: send specific invalidation type to jquery.dataTables library 
		$.fn.dataTable.Api( oSettings ).rows().invalidate('colreorder');
...
//		<ColReorder compatibility changes> ab/082020 patch for 'ColReorder' library: send specific invalidation type to jquery.dataTables library 
		$.fn.dataTable.Api( this.s.dt ).rows().invalidate('colreorder');
	 ...
	 *  @private
	 */
	// <ColReorder compatibility changes> ab/082020: prevent from current column be able to be dragged(reordered) in case the column contains class 'column-reorder-disabled'
	"_fnMouseListener": function ( i, nTh )
	{
		var that = this;
		$(nTh)
			.on( 'mousedown.ColReorder', function (e) {
				if ( that.s.enable && e.which === 1 && e.currentTarget.className.indexOf('column-reorder-disabled') == -1) {
					that._fnMouseDown.call( that, e, nTh );
				}
			} )
			.on( 'touchstart.ColReorder', function (e) {
				if ( that.s.enable && e.currentTarget.className.indexOf('column-reorder-disabled') == -1) {
					that._fnMouseDown.call( that, e, nTh );
				}
			} );
	},
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		$(document)
			...
			.on( 'mouseup.ColReorder touchend.ColReorder', function (e) {
				...
				console.log("document mouseup.ColReorder touchend.ColReorder");
				
				// <ColReorder compatibility changes> ab/082020: 
				// store the columns order in the global variable, created in ElementDataTableApiImp.js				
				var oSettings = _dtTable.dt;
				var columns = oSettings.aoColumns;
				var tableId = oSettings.sTableId;
				var _arr = [];
	    		for(var i=0;i < columns.length;i++) {
	    			var _n = columns[i].uniqueTitle;
	    			_arr[i] = _n.trim();
	    		}
	    		globalDataTableColumnsOrderHolder[tableId] = _arr;
//	    		console.log(globalDataTableColumnsOrderHolder[tableId]);
	    		// redraw table
	    		_dtTable.dt.oInstance.fnDraw(false);   		
	    		// init dtable Fixed Headers
	            initFixedHeaders(tableId);
	            ///////////////////////////
			} );
	},
