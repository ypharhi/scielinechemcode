...
(function( factory ) {
    ...
    $.extend( ColResize.prototype, {
        ...
        _defaults: {
            ...
            resizeTable: false,
            /**
             * Datatables instance ID
             * @property _tableInstanceID
             * @type {string}
             * @public
             * @default {null}
             */
        ...
        */
        ...
        init: function(dtInstance) {
            ...
            that._tableInstanceID = that._table.attr('id');
...
//                console.log("on table destroy event", that._table[0]);
            	that.destroy();
            }, true);
        },
        /**
...
//        		console.log("---- INIT column resizable on: ",that._table[0]);
//                console.log("table width", that._table.width());   
            ...
            // get any saved width from the local storage
            defaultWidths = that.getLocalStoredWidths();
...
//            }
            ...
            var thatTableOuterHeight = that.checkTableHeight();
            $(that._container).attr('resizableDivHeight',thatTableOuterHeight);
            that.initEvents();
        },
        /**
...
//        },
        /**
         * Get saved widths for columns that was resized
         * @returns {id:value|null}
         */
        getLocalStoredWidths: function() {
            var that = this;
            var _table = that._table;
            var _tableId = that._table.attr('id');
            var _tableColWidth = globalDataTableColumnWidthHolder[_tableId];
//            console.log("_tableColWidth",_tableColWidth);
            if(_tableColWidth && Object.keys(_tableColWidth).length > 0)
			{
				return _tableColWidth;
			}
        },
        
        saveWidthData :function(colIndex, newWidth) {
        	var that = this;
        	
            var _table = that._table;
            var $th = that._tableHeaders.eq(colIndex);
            var $thName = $th.attr('uniquetitle');
            var _tableId = _table.attr('id');
            if(globalDataTableColumnWidthHolder[_tableId]) {
            	globalDataTableColumnWidthHolder[_tableId][$thName] = newWidth;
            }
            else {
            	globalDataTableColumnWidthHolder[_tableId] = {};
            	globalDataTableColumnWidthHolder[_tableId][$thName] = newWidth;
            }
        },
        /**
...
//                console.log('Table redrawn', that._table[0]);
            ...
            that._addEvent(that._table, "column-visibility.dt", function() {
                ...
                initFixedHeaders(that.options.tableInstanceID);
            }, true);
        ...
        calcTableDimensions: function(widths) {
            ...
            for (var i = 0, l = $ths.length; i < l; i++) {
                ...
                $thName = $th.attr('uniquetitle');
                // get <th> current/correct width
                if(widths && widths.hasOwnProperty($thName))
                {
                	thWidth = widths[$thName];
                }
                else
                {
                	thWidth = that._getWidth($th); 
                	//if(thWidth == 0) thWidth = 120;
                }
            ...
            //console.log("calcTableDimensions", totalWidth);
            // and it's container
            if (that.options.scrollY) {
                totalWidth = totalWidth + 20;
            }
            that._container.width(totalWidth + 5); // added 5px for last column improve drag catch event
        },
        /**
        ...
        _getWidth: function($th) {
            ...
            if (typeof $th[0].style.width === "string" && $th[0].style.width.indexOf("px") >= 1) 
            {
               ...
               // console.log("$th[0].style.width: ", $th[0].style.width);
            } 
            else 
            {
                width = $th.outerWidth();    
                //console.log("$th.outerWidth(): ", $th.outerWidth());
            }
//            console.log("----------------------------------------");
           // console.log("outer width:" + $th.outerWidth()," / style width:"+$th[0].style.width);
//            console.log("return width", width);
//            if(width == 0)return width;
        ...
        onMouseDown: function() {
            ...
            return function() {
                ...
                $(document).one("mouseup", function() {
                    ...
                    var $colnewwidth = $col.data(that.DATA_TAG_ITEM).outerWidth();
                    that.saveWidthData($col.index(),$colnewwidth);
                    $(that._table).trigger($.Event( "column-resized.dt" ), [$col.index(), $colnewwidth] );
                    //that._dtInstance.state.save();
        ...
        onMouseMove: function(event, $col) {
            ...
            if (that._isDragging) {
            	...
            	diff = Math.ceil(event.pageX - $col.offset().left);                
                //console.log("diff=(event.pageX - $col.offset().left): ",diff);
                
                ////temporary solution for the next problem: when scroll is reaching right side of the screen, column resizing performed with issues////
            	var _scrollLeft = $(window).scrollLeft();
            	var _winWidth = $(window).width();
            	var _docWidth = $(document).width();
            	//var _d = _docWidth - (_winWidth + _scrollLeft);
            	if(_scrollLeft > 0  && (_docWidth == _winWidth + _scrollLeft) && diff < -5)
            	{
            		diff = -3;
            		//$(window).scrollLeft($(window).scrollLeft() - 30); 		
            	}
            	////////////////////////////////////////////////////////////////////////////
        ...
        updateColumn: function($col, by, nextColumn) {
            ...
            if (newWidth > that.options.minColumnWidth) {
                ...
                if(nextColumn) {
                    // set the new left position of the dragged column (div)
        ...
        checkTableHeight: function() {
            ...
            if (newHeight !== that._tableHeight) {
                ...
                //console.log("table.position().top: ", that._table.position().top);
                //console.log("table.offset().top",that._table.offset().top)
               ...
               /* that._columns.offset({
                    'top': that._table.offset().top
                });*/
            }
           return newHeight; 
        },
        ...
        redraw: function() {
            var that = this;
//            console.log("redraw => column resizable");
            that.destroy();
    ...
    $(document).on("init.dt.dtr", function(e, settings) {
        ...
        //if(init !== false) {
        if(init) {
//        	console.log("on 'init.dt.dtr' event", settings);
            ...
            setTimeout(function() {
                ...
                var tID = init.tableInstanceID;
                // init dtable Fixed Headers
                initFixedHeaders(tID);
                
                $('#'+tID).one('mouseenter', 'th, tbody', function() 
                {
		    		console.log("mouseenter");	
		    		var _dt = new DataTable.Api(settings);
		    		_dt.colResize.redraw();
			    });
            },1);
        }
    });
    ...
    $(window).bind("load", function() {
		...
		//var $tables = $('table.dataTable.editable');
		var $tables = bl_getResizableTables();
//		console.log("*** Resizable tables: ",$tables);
		if($tables.length > 0)
		{
			$tables.each(function(index)
			{
				var $table = $($tables[index]);
				var _tableOuterHeight = $table.outerHeight();
				var _rszDivHeight = $table.parent().find('div.dt-colresizable').attr('resizableDivHeight');
				
				var _isDataTable = $.fn.DataTable.isDataTable($table[0]);
//				console.log("table: ",$table[0]);
//				console.log("isDataTable",_isDataTable);
//				console.log(" outerHeight: "+ _tableOuterHeight + " resizable divs height: "+ _rszDivHeight);
				
				var _dtable = null;
				if(_isDataTable)
				{
					_dtable = $table.DataTable();
					
					if(_dtable.settings().data().length > 0)
		    		{					
						/** Unbind event listener */
						//$table.off('mouseenter', 'th, tbody');
						
						/** Redraw table **/
						/** if table has data it has outer height, 
						*   	but resizable divs still hasn't height (when calculation of table height happened before table was rendered on screen)  
						*/
						if(_tableOuterHeight > 0 && _rszDivHeight == "0")
			    		{
							_dtable.colResize.redraw();
	//		    			 new $.fn.dataTable.ColResize( _dtable, {
	//		    				resizeTable : true
	//		    			} ); 
			    		}
						/** if table has data, but table outerHeight func returns '0' 
						*   =>  may be caused if table is located in tab different from loaded tab, that tab content is hidden and has 'display:none' property
						*/
						else if(_tableOuterHeight == 0)
			    		{
//							commented, because relocated on after init table
//							$table.one('mouseenter', 'th, tbody', function() {
//						    		console.log("mouseenter");						    		
//						    		_dtable.colResize.redraw();
//						    });
			    		}
...
//				else /** in case LOAD is happened before table is initialized as jquery datatable object */
//				{
//					/** 
//					 * 	if table has data, but table outerHeight func returns '0' 
//					*   =>  may be caused if table is located in tab different from loaded tab, that tab content is hidden and has 'display:none' property
//					*/
//					if(_tableOuterHeight == 0)
//		    		{
//						$table.one('mouseenter', 'th, tbody', function() {
//					    		console.log("mouseenter");	
//					    		var _dtable = $table.DataTable();
//					    		if(_dtable && _dtable.settings().data().length > 0)
//					    		{
//					    			_dtable.colResize.redraw();
//					    		}
//					    });
