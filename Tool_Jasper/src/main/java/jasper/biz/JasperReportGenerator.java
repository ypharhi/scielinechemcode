...
//	String userPassword = ""; 
	...
	public ByteArrayOutputStream getByteArrayOutputStream(String sReportID, String sReportName, JasperReportType reportType, String sReportParameterTitle, String sReportParameterFilterDisplay, HashMap<String, String> hmReportReplacerList, HashMap<String, Object> hmReportParameterList, String reportPath, String reportPathTmp, boolean useVirtualizer) throws Exception
	{
		//init..
                        ...
                        switch (reportType)
                        {
                            case PDF: 
                            {
                                // Add arial font for pdf
                                 ...
                                 PdfFont font = new PdfFont( reportPath + "\\ARIAL.TTF",BaseFont.IDENTITY_H,true); //(String pdfFontName, String pdfEncoding, Boolean isPdfEmbedded)  
                            ...
                            case SIMPLE_EXCEL: 
                            {
                                    ...
                                    // trying to guess the number columns  from the query that need to be shown in the excel 
                                    int colCount;
                                    ...
                                    if (map.containsKey("ReportColumnExportCSV")) // take the columns from SCV parameter
                                    {
	                                		...
	                                		reportColumnExportCSV = (jrpReportColumnExportCSV.getDefaultValueExpression().getText()).replaceAll("\"", "");
	                                		colCount = (reportColumnExportCSV.split(",")).length; 
                                    		
                                    }
                                    else if (map.containsKey("ReportColumnExportCounter")) // see if it defined as parameter (number in string type) -> take first ReportColumnExportCounter from report header
                                    {
                                    ...
                                    sbExcel.append("<tr>\n<td colspan='" + colCount + "' style='font-weight:bold'>").append(mLang != null ? generalBiz.modifiedLangOnString(sReportParameterTitle,mLang) : sReportParameterTitle).append("</td>\n</tr>\n");
                                    sbExcel.append("<tr>\n<td colspan='" + colCount + "' style='font-weight:bold'>").append(mLang != null ? generalBiz.modifiedLangOnString(sReportParameterFilterDisplay,mLang): sReportParameterFilterDisplay).append("</td>\n</tr>\n");
                                    sbExcel.append("<tr>\n");
...
//                        }
                ...
                else // use default getByteArrayOutputStream that invoke getPath (=> uses virtualizer on big data)   
                {
                    toReturn = getByteArrayOutputStream(sReportID,sReportName,reportType,sReportParameterTitle,sReportParameterFilterDisplay,hmReportReplacerList,hmReportParameterList,reportPath,reportPathTmp);
                }
		return toReturn;
	}
        ...
        public ByteArrayOutputStream getByteArrayOutputStream(String sReportID, String sReportName, JasperReportType reportType, String sReportParameterTitle, String sReportParameterFilterDisplay, HashMap<String, String> hmReportReplacerList, HashMap<String, Object> hmReportParameterList, String reportPath, String reportPathTmp) throws Exception 
        {
            ...
            String path = getPath(sReportID,sReportName,reportType,sReportParameterTitle,sReportParameterFilterDisplay,hmReportReplacerList,hmReportParameterList,reportPath,reportPathTmp);
	...
	public String getPath(String sReportID, String sReportName, JasperReportType reportType, String sReportParameterTitle, String sReportParameterFilterDisplay, HashMap<String, String> hmReportReplacerList, HashMap<String, Object> hmReportParameterList, String reportPath, String reportPathTmp) throws Exception
	{
		//init.. 
//		StringBuilder rNameToCompile = new StringBuilder(sReportName);
		String toReturn = reportPathTmp + "\\" + sReportID+ (new Date()).getTime(); // add time in case some call forgot that it's unique
		JasperReport report = null;
		InputStream input = null;
		InputStream inputModified = null;
		JasperDesign design = null;
		...
		try
		{
		    ...
		    JRSwapFile swapFile = new JRSwapFile(reportPathTmp, VIRTUALIZER_BLOCK_SIZE, VIRTUALIZER_MIN_GROW_COUNT);
                    virtualizer = new JRSwapFileVirtualizer(VIRTUALIZER_MAX_PAGE_SIZE_CACHE, swapFile, true); //if more than VIRTUALIZER_MAX_PAGE_SIZE_CACHE pages we start saving data in the disck
                    hmReportParameterList.put(JRParameter.REPORT_VIRTUALIZER, virtualizer); 
			...
			{  
				...
				exporter.exportReport();	
/*                            
                                //render report without the font map
                                toReturn = toReturn + ".pdf";
                                input = new FileInputStream(new File(reportPath + "\\" + sReportName));
                                inputModified = generalBiz.InputStreamModified(reportPath,input, hmReportReplacerList);
                                design = JRXmlLoader.load(inputModified);
                                input.close();
                                inputModified.close();
                                report = JasperCompileManager.compileReport(design);
...
//		        }
		    ...
		    {
			System.out.println("JasperReportGenerator.getPath - close connection failed conn=" + conn + ", e=" + e.toString());
		    }
		}
		return toReturn;
	}
	
	public String getCompiledPath(String sReportID, String sReportName, String reportPath, String reportPathTmp, HashMap<String, String> hmReportReplacerList) throws Exception
	{
		//init.. 
		GeneralBiz generalBiz = new GeneralBiz(); 
//		StringBuildesReportNamele = new StringBuilder(sReportName);
		 ...
		 if(jrDataSource == null)   //DB connection -> print the sql
                 {
                     try
                     {
                         ...
                         System.out.println("Sub report query: " + sFileContenet.substring(sFileContenet.indexOf("<queryString>") + "<queryString>".length(),sFileContenet.indexOf("</queryString>"))); 
                     }
                     catch (Exception e)
                     {
        ...
        public JasperReport getCompiled(String sReportID, String sReportName, String reportPath, String reportPathTmp, HashMap<String, String> hmReportReplacerList) throws Exception
        {
        	...
        	if(!generalBiz.isGenerateCompileFileOnDevelop(reportPath)) {
        		...
        		try {
        			...
        			if (reportCompiled.exists()) {
        				toReturn = (JasperReport)JRLoader.loadObject(new File(reportPath + "\\" + sReportName.replace(".xml", "") + ".jasper"));//TODO check it!!!!!
        				return toReturn;
        			}
        		} catch (Exception e) {
        			toReturn=null;
